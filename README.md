...
This application was generated using JHipster 8.10.0 and JHipster .Net Core , you can find documentation and help at https://jhipsternet.readthedocs.io/en/latest/index.html and [https://www.jhipster.tech/documentation-archive/v8.10.0](https://www.jhipster.tech/documentation-archive/v8.10.0).
...
- `src/Dotnetfinal/ClientApp/` - Web Application folder
...
In ./src/Dotnetfinal/ClientApp run
...
auto-refreshes when files change on your hard drive.
    ...
    npm --prefix ./src/Dotnetfinal/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/Dotnetfinal/ClientApp run` command will list all of the scripts available to run for this project.
Before you can build this project, you must install and configure the following dependencies on your machine:
...
The service worker initialization code is commented out by default. To enable it, uncomment the following code in `src/Dotnetfinal/ClientApp/src/index.html`:
...
To build the artifacts and optimize the dotnetfinal application for production, run:

```
cd ./src/Dotnetfinal
rm -rf ./src/Dotnetfinal/wwwroot
dotnet publish --verbosity normal -c Release -o ./app/out ./Dotnetfinal.csproj
```

The `./src/Dotnetfinal/app/out` directory will contain your application dll and its depedencies.
...
3. Run ``dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"Dotnetfinal" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml"``
...
```bash
docker build -f ./Dockerfile-Back -t dotnetfinal .
```

Then run:

```bash
docker run -p 8080:8080 dotnetfinal
```
