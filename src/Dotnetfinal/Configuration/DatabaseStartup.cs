using System;
using Dotnetfinal.Infrastructure.Data;
...
public static class DatabaseConfiguration
{
    public static IServiceCollection AddDatabaseModule(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        {
            connectionString = configuration.GetSection("EntityFramework")["DataSource"];
        }

        // Decidir qué proveedor de base de datos usar
        ...
        {
            // SQL Server
            services.AddDbContext<ApplicationDatabaseContext>(context => { context.UseSqlServer(connectionString); });
        }
        else
        {
            // SQLite con archivo
